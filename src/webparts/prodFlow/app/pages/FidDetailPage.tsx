...
import DetailSideNav, {
  NavItemState,
} from "../components/layout/DetailSideNav";
...
// Marca o FID que precisou de revisão na orçamentação — usado para contabilização depois.
const BudgetRevisionChip: React.FC<{ data: IFabricationRequest }> = ({
  data,
}) => {
  const stats = data.revisaoOrcamento;
  if (!stats?.houve) return null;
  return (
    <span
      className={`${styles.deadlineChip} ${styles.chipDue}`}
      title={`Revisões na orçamentação: ${stats.total} (Delin. Fabricação ${stats.porEtapa.delineation} · Cotações ${stats.porEtapa.quotations})`}
    >
      <ArrowSync16Filled />
      {stats.total === 1 ? "1 revisão" : `${stats.total} revisões`}
    </span>
  );
};
...
const OverviewTab: React.FC<{ fid: string; data: IFabricationRequest }> = ({
  ...
  const canEditTopLevelDrawing = isAdmin || teams.indexOf("planning") >= 0;
  return (
    <div className={styles.stack}>
      ...
      <div className={styles.overviewGrid}>
        ...
        <FidDrawingCard data={data} compact canEdit={canEditTopLevelDrawing} />
...
export const FidDetailPage: React.FC = () => {
  ...
  const badges = React.useMemo<Partial<Record<FidTabKey, string>>>(() => {
    ...
    const undefinedStrategy = count(
      (s) => !s.strategy && !s.engAnalysis?.requestedAt,
    );
    const pendingAnalysis = count(
    ...
    return {
      ...
      delineation:
        pendingAnalysis + pendingDelineation > 0
          ? String(pendingAnalysis + pendingDelineation)
          : undefined,
  ...
  >(() => {
    ...
    return {
      ...(strategiesDone ? { subitems: "done" as const } : {}),
      ...budgetStageNavState(data),
    };
  }, [data]);
      ...
      <header className={styles.hero}>
        <div className={styles.heroMain}>
          ...
          <div className={styles.heroMeta}>
            ...
            <BudgetRevisionChip data={data} />
          </div>
          <div className={styles.heroProject}>{data.tituloProjeto || "—"}</div>
